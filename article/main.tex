...
The vortex-cluster analysis of \citet{DelAlamoEtAl2006} separates compact detached objects from tall attached objects.  Figure~\ref{fig:vortex-clusters} illustrates that the attached cluster is not a single idealised arch but an internally complicated object.  Its Cartesian bounding dimensions scale approximately as
\begin{equation}
...
\begin{figure}[tb]
 \centering
 \includegraphics[width=0.88\linewidth]{vortex-clusters.png}
 \caption{Representative detached (left) and tall attached (right) vortex clusters.  The tall cluster is a multiscale connected object rather than one idealised vortex arch. Reproduced from figure~6 of \citet{DelAlamoEtAl2006}.}
 \label{fig:vortex-clusters}
\end{figure}
