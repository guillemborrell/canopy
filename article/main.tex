...
The tracked structures therefore introduce neither a separate outer length nor a separate outer time scale: their dimensions scale with $\Delta_y$, and their duration with $\Delta_y/u_\tau$. This is the role of the time-resolved evidence in the hypothesis: the coherent structures can form a canopy while preserving wall similarity. The identification of that canopy with the boundary-layer interface is then supplied by the continuity evidence in section~\ref{sec:interface}.

\FloatBarrier
\section{Continuity across the established T/NT interface}
\label{sec:interface}
...
The vorticity amplitude is extinguished across the edge without a corresponding replacement of the local strain geometry. This supplies the interface side of the hypothesis: the edge inherits the internal turbulent straining environment expected if it is the outer envelope of the cluster hierarchy. Together with the wall-similar hierarchy in section~\ref{sec:hierarchy}, it motivates the cluster-to-interface identification advanced here.
...
The main value of the canopy framework is not that it abolishes the outer layer. It identifies the wall-similar attached hierarchy with the structural support of the T/NT interface, identifies figures~\ref{fig:outer-strain} and \ref{fig:interface-straining} as the strongest existing evidence for that connection, and turns the hypothesis into measurable predictions.
