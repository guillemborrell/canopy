...
Planar particle-image velocimetry established that the outer TBL contains repeated combinations of spanwise vorticity and strong Q2 motion \citep{AdrianEtAl2000}.  Three-dimensional direct numerical simulation (DNS) subsequently showed that intense Reynolds-stress regions and vortex clusters include wall-attached, approximately self-similar families \citep{DelAlamoEtAl2006,LozanoDuranEtAl2012}.  Time-resolved DNS showed that the geometrical similarity of tall attached structures extends to their lifetimes \citep{LozanoDuranJimenez2014}.  In parallel, conditional analysis of the turbulent/non-turbulent (T/NT) interface showed a sharp separation between turbulent-level gradients and a much weaker potential-flow strain branch \citep{BorrellJimenez2016}.
...
We use the interface-conditioned statistics reported by \citet{BorrellJimenez2016} directly. The present question is what those statistics imply for the canopy framework.
...
This supplies the interface side of the hypothesis: the wall-similar cluster hierarchy retains its internal organisation up to the turbulent side and then terminates in a sharp amplitude drop. The T/NT interface is proposed to be the envelope of those terminations.


\FloatBarrier
\section{The cluster-to-interface hypothesis}
\label{sec:synthesis}
...
\begin{enumerate}
 ...
 \item turbulent-level vorticity and strain terminate at the boundary; core-like normalised straining persists up to its turbulent side, while only a much weaker potential branch remains outside.
\end{enumerate}
The central hypothesis of this paper is therefore that the intermittent region is the sparse upper support of the wall-attached hierarchy, and that the T/NT interface is its convoluted outer envelope rather than a separate sheet placed above the outer flow. This also explains why a planar section can suggest isolated bulges or packets even when the three-dimensional field is strongly connected.

A joint object/interface analysis provides a direct test of the hypothesis: vorticity clusters, Q structures and the interface should be extracted simultaneously from the same TBL snapshots and their conditional intersections measured. The observation that more than 80\% of channel vortex clusters intersect a Q object supplies a concrete expectation for that test.
...
\begin{enumerate}
 ...
 \item \textbf{Wall similarity.} Tracked tall attached structures preserve geometrical and temporal similarity: their dimensions scale with height and their lifetimes with $\Delta_y/u_\tau$. They can therefore populate a canopy without introducing a new outer scale.
 ...
 \item \textbf{Cluster-to-interface hypothesis.} The T/NT interface is proposed to be the convoluted outer envelope of the sparse wall-attached cluster hierarchy. This identification unifies the evidence above and predicts measurable joint cluster/interface intersections.
\end{enumerate}

The main value of the canopy framework is not that it abolishes the outer layer. It identifies the wall-similar attached hierarchy with the structural support of the T/NT interface, identifies figures~\ref{fig:outer-strain} and \ref{fig:interface-straining} as evidence respectively for sharp termination and core-like organisation up to the turbulent side, and turns the hypothesis into measurable predictions.
