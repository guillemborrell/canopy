...
Let $\omega=|\bm{\omega}|$ be the vorticity magnitude. We adopt without modification the vorticity-interface convention and interface-conditioned coordinate established by \citet{BorrellJimenez2016}. Their dimensionless interface scale is
\begin{equation}
...
\FloatBarrier
\section{Termination at the established T/NT interface}
\label{sec:interface}
...
\begin{figure}[tb]
 ...
 \caption{Joint wall-normal distributions of vorticity (blue dashed contours) and strain (black solid contours). Turbulent-level strain drops by more than one order of magnitude near the edge; the remaining weak potential branch follows the exponential decay marked in red. This is a separation of regimes, not continuous turbulent strain into the free stream. Reproduced from figure~20 of \citet{BorrellJimenez2016}.}
...
\begin{enumerate}
 ...
 \item Turbulent-level vorticity and strain then terminate sharply at the T/NT interface; the exterior branch is much weaker and potential.
\end{enumerate}
...
\begin{enumerate}
 ...
 \item \textbf{Termination at the edge.} Turbulent-level vorticity and strain terminate near the interface in a drop of more than one order of magnitude. Core-like normalised straining persists up to the turbulent side, while only a weak exponentially decaying potential branch remains outside.
