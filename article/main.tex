...
\subsection{Compatibility with wall similarity}

The time-resolved question relevant here is only whether coherent structures can populate the canopy without breaking wall similarity. \citet{LozanoDuranJimenez2014} found that tracked tall attached branches remain geometrically self-similar and that their lifetimes scale as
\begin{equation}
...
The tracked structures therefore introduce neither a separate outer length nor a separate outer time scale: their dimensions scale with $\Delta_y$, and their duration with $\Delta_y/u_\tau$. They are compatible with a canopy that preserves wall similarity. Because the tracking was performed in channels and did not label a boundary-layer interface, it establishes compatibility rather than the missing cluster-to-canopy connection.
